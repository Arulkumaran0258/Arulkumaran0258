...
def find_nearby_cities_route():
    ...
    if location:
        ...
        if nearby_cities:
            return jsonify({'nearbyCities': nearby_cities})
        else:
            # If no nearby cities found, search in database for matching locations
            conn = mysql.connector.connect(**db_config)
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT * FROM park_owners WHERE location LIKE %s", ('%' + location + '%',))
            matched_places = cursor.fetchall()
            cursor.close()
            conn.close()

            if matched_places:
                return jsonify({'matchedPlaces': matched_places})
            else:
                return jsonify({'error': 'No nearby locations available in our database.'})
    else:
